...
const bookingSchema = z
    .object({
        bookingStart: z.instanceof(Date).nullable().optional(),
        ...
        institutionName: z.string().min(1, "Nama Unit Kerja wajib diisi"),
        purpose: z.string().max(500).min(1, "Nama Kegiatan wajib diisi"),
...
class SupabaseRoomRepository implements IRoomRepository {
    ...
    async getRoomsForRange(): Promise<Room[]> {
    ...
    private mapToEntity(d: any): Room {
...
class SupabaseBookingRepository implements IBookingRepository {
    ...
    async createBooking(
        ...
        // Public booking: No auth check required
        const newBooking = {
    ...
    async listBookingsOverlapping(
        ...
        const results: Booking[] = data.map((d: any) => this.mapToEntity(d));
    ...
    private mapToEntity(d: any): Booking {
        const p = d.payload || {};
...
const ActivityNameSection: React.FC<{
    form: UseFormReturn<BookingFormData>;
}> = ({ form }) => {
    const {
        register,
...
const RoomBookingSystem: React.FC = () => {
    ...
    const {
        handleSubmit,
        watch,
        // setError, // No longer needed for manual validation
        reset,
    ...
    // Removed manual useEffect validation to rely on Zod schema
