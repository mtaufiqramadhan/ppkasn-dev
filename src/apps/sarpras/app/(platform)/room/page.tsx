...
const parseFloor = (value: number | string) => {
  ...
  // Extract number from strings like "Lantai 2", "Lt. 3", "Lantai 10", etc.
  const match = s.match(/\d+/);
  if (match) {
    return { kind: "num", num: Number(match[0]) } as const;
  }
...
const compareRooms = (a: Room, b: Room): number => {
  ...
  // 1. Urutkan berdasarkan lantai numerik (lantai 1, 2, 3...)
  if (fa.kind === "num" && fb.kind === "num") {
    if (fa.num !== fb.num) {
      return fa.num - fb.num;
    }
    // Jika lantai sama, urutkan berdasarkan nama ruangan secara alfabetis (A - Z)
  ...
  // Jika kedua lantai berupa teks non-angka
  const floorCmp = COLLATOR.compare(fa.text, fb.text);
  if (floorCmp !== 0) {
    return floorCmp;
  }

  // Jika teks lantai sama, urutkan berdasarkan nama ruangan (A - Z)
  return COLLATOR.compare(a.name, b.name);
};
