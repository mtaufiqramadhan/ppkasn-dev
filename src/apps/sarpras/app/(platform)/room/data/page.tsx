...
export default function RoomBookingsDataPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch rooms with type in ['ruangan', 'ruang']
      ...
      const loadedBookings: Booking[] = (bookingsData || [])
        ...
        // Only include bookings associated with these rooms
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      toast.success("Peminjaman ruangan berhasil diperbarui");
      setIsEditOpen(false);
      loadData();
    } catch (e) {
      const message = e instanceof Error ? e.message : "Gagal memperbarui peminjaman";
      toast.error(message);
    } finally {
      setIsSaving(false);
    }
  ...
  const handleExportPDF = () => {
    ...
    // Add Title
    doc.setFontSize(16);
    doc.text("Laporan Peminjaman Ruangan", 14, 20);
    doc.setFontSize(10);
    ...
    doc.save(`laporan-peminjaman-ruangan-${format(currentDate, "MM-yyyy")}.pdf`);
  };
    ...
    <div className="container mx-auto py-6 px-3 sm:px-4 md:px-6 mb-6 max-w-7xl">
                ...
                <TableRow>
                  <TableCell colSpan={8} className="h-32 text-center">
                    <div className="flex flex-col items-center justify-center gap-2 text-slate-500">
                      <p className="text-sm font-medium">Tidak ada data peminjaman ruangan</p>
                    ...
                    >
                      ...
                      <TableCell>
                        <span className="inline-flex items-center rounded-md bg-slate-50 border border-dashed border-slate-300 px-2 py-1 text-xs font-medium text-slate-600">
                          {roomNames || "Ruangan tidak ditentukan"}
                        </span>
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="sm:max-w-[425px] rounded-xl border border-dashed border-slate-300 shadow-none bg-white">
          <DialogHeader>
            <DialogTitle>Edit Peminjaman Ruangan</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label htmlFor="bookingStart" className="text-xs font-semibold">Tanggal Mulai</Label>
                <Input
                  id="bookingStart"
                  type="date"
              ...
              <div className="grid gap-2">
                <Label htmlFor="bookingEnd" className="text-xs font-semibold">Tanggal Selesai</Label>
                <Input
                  id="bookingEnd"
                  type="date"
                  className="rounded-xl border-dashed border-slate-300 shadow-none focus:ring-0 focus:border-slate-400 text-xs h-9"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label htmlFor="startTime" className="text-xs font-semibold">Waktu Mulai</Label>
                <Input
                  id="startTime"
                  type="time"
                  className="rounded-xl border-dashed border-slate-300 shadow-none focus:ring-0 focus:border-slate-400 text-xs h-9"
              ...
              <div className="grid gap-2">
                <Label htmlFor="endTime" className="text-xs font-semibold">Waktu Selesai</Label>
                <Input
                  id="endTime"
                  type="time"
                  className="rounded-xl border-dashed border-slate-300 shadow-none focus:ring-0 focus:border-slate-400 text-xs h-9"
            ...
            <div className="grid gap-2">
              <Label htmlFor="name" className="text-xs font-semibold">Nama Peminjam</Label>
              <Input
                id="name"
                className="rounded-xl border-dashed border-slate-300 shadow-none focus:ring-0 focus:border-slate-400 text-xs h-9"
                value={editForm.name || ""}
      ...
      <AlertDialog open={!!deletingId} onOpenChange={(open) => !open && setDeletingId(null)}>
        <AlertDialogContent className="rounded-xl border border-dashed border-slate-300 shadow-none bg-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Data peminjaman ruangan akan dihapus permanen dari sistem.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
